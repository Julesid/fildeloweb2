...
router.use(session({ secret: 'ton_secret', resave: false, saveUninitialized: true }));
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const isPasswordValid = await bcrypt.compare(password, utilisateur.mdp_bcrypt);
...
// Route pour récupérer les promotions depuis la base de données
router.get('/promotion', async (req, res) => {
  try {
    // Tente de récupérer les promotions depuis la base de données
    const promotions = await Promotion.findAll();

    // Si des promotions sont récupérées, les envoyer au frontend
    if (promotions && promotions.length > 0) {
      res.json(promotions);
    } else {
      // Si aucune promotions n'est trouvée, renvoyer des promotions par défaut
    ...
    // En cas d'erreur, renvoyer des promotions par défaut
    const defaultPromotions = [
      { id: 1, label: 'Promotions 1' },
      { id: 2, label: 'Promotions 2' },
      { id: 3, label: 'Promotions 3' }
